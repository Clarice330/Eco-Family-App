...
# ==================== 7. 功能頁面 2：📍 親友共享定位雷達 ====================
    ...
    if not st.session_state.joined_room:
        ...
        # 採用長亮穩定連線之純 Leaflet 繪製（零強制斷開，零彈出錯誤）
        map_html_template = """
...
# ==================== 10. 功能頁面 5：🚨 全國 SOS 緊急求救專區 (一鍵發送 SMS 簡訊) ====================
    ...
    # 🎯 純前端即時渲染 SMS 按鈕（100% 避免連線中斷或網頁重刷崩潰）
    sos_gps_js_template = """
    ...
    <script>
        function updateSmsButton(lat, lon) {
            ...
            // 判斷 iOS 與 Android 的 SMS URL 參數分隔符規格
    ...
    st.info("💡 提示：點擊上方 SMS 簡訊發送按鈕，手機會自動開啟簡訊文字檔，按下發送即可迅速向搜救隊通報您的精確位置！")dfdsgfdrestestreretraet