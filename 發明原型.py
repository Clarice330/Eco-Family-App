...
# 讀取 URL 參數進行跳轉（確保四大按鈕 100% 採用大卡片格式）
query_params = st.query_params
if "page" in query_params:
    target_p = query_params["page"]
...
# ==================== 2. 全局 CSS 樣式美化 (四大功能按鈕 100% 統一為大卡片格式) ====================
st.markdown("""
<style>
    .stApp {
    ...
    /* 四大功能按鈕 100% 統一為大卡片按鈕格式 (高76px、圓角16px、深綠文字、無下劃線、無箭頭) */
    .unified-card-btn {
        width: 100%;
    ...
    .unified-card-btn:visited, .unified-card-btn:hover, .unified-card-btn:active {
        color: #1B5E20 !important;
        text-decoration: none !important;
        border-bottom: none !important;
    }
...
# ==================== 5. 頁面 1：主選單 (按鈕2一按即直接跳轉 Google 地圖共享定位) ====================
if st.session_state.current_page == "menu":

    # 卡片按鈕 1
    st.markdown("""
    <a href="?page=routes" target="_self" class="unified-card-btn">
        🗺️ 智慧路線規劃
    </a>
    """, unsafe_allow_html=True)

    # 卡片按鈕 2 (一按直接切換至 Google 地圖共享定位網頁)
    google_maps_sharing_url = "https://www.google.com/maps/@22.1987,113.5439,15z"
    st.markdown(f"""
    <a href="{google_maps_sharing_url}" target="_blank" class="unified-card-btn">
        📍 親友共享定位雷達
    </a>
    """, unsafe_allow_html=True)
    ...
    # 卡片按鈕 3
    st.markdown("""
    <a href="?page=gear" target="_self" class="unified-card-btn">
        🎒 隨行裝備
    </a>
    """, unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <a href="{ext_url}" target="_blank" class="unified-card-btn">
        🔍 親子生態動植物識別
    </a>
    """, unsafe_allow_html=True)
...
# ==================== 6. 功能頁面 1：🗺️ 智慧路線規劃 (18條精確條件匹配路線) ====================
    ...
    with st.expander("🛠️ 手動氣象模擬"):
        ...
        if st.session_state.override_weather:
            ...
            st.session_state.global_rain = st.checkbox("🌧️ 模擬降雨", value=st.session_state.global_rain, key="r_rain")
    ...
    with r4:
        ...
        st.markdown(f"""<div class="metric-card"><div class="metric-title">🌧️ 天氣</div><div class="metric-value" style="font-size:1.1rem; margin-top:2px;">{rain_text}</div></div>""", unsafe_allow_html=True)
    ...
    <div class="card">
        ...
        <p style="font-size:0.9rem; margin-bottom:0;">選擇澳門目的地，系統將依據<b>當前氣溫、紫外線與降雨</b>推薦最適路線：</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    for r in dest_routes:
        ...
        # 精確獨一無二算分矩陣：保證第一名隨著氣象數據 100% 動態翻轉，無同分競爭！
        if is_rain:
            if cond == "rain":
                score = 98.0
            elif cond == "hot":
...
# ==================== 8. 功能頁面 3：🎒 隨行裝備 (有需要才精準動態顯示，無雨絕對不顯示雨具) ====================
    ...
    with st.expander("🛠️ 手動氣象模擬"):
        ...
        if st.session_state.override_weather:
            ...
            st.session_state.global_rain = st.checkbox("🌧️ 模擬降雨", value=st.session_state.global_rain, key="g_rain")
    ...
    <div class="card">
        ...
        <p style="font-size:0.9rem; margin-bottom:0;">系統根據目前的<b>氣溫、紫外線、降雨與懸浮微粒</b>數據精算出的推薦清單：</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.session_state.global_rain:
        st.markdown("##### 🌧️ 降雨天氣專屬加強裝備 (當前降雨中)")
    ...
    # 5. 😷 PM2.5 / PM10 懸浮微粒動態應對裝備 (僅在微粒偏高時顯示)
    cur_pm25 = st.session_state.global_pm25
    if cur_pm25 >= 15.0:
        st.markdown(f"##### 😷 呼吸道護理裝備 (當前 PM2.5 {cur_pm25:.1f})")
