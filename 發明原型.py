...
標準配置：3 大功能 + 2 小功能 (右上角配置) + 頂部關愛大字體模式開關
"""
...
# ==================== 2. 全局 CSS 樣式美化 (主按鈕 + 內文同步放大) ====================
is_elder = st.session_state.is_elder_mode

# 動態計算各種層級的字體大小
btn_font_size = "2.0rem" if is_elder else "1.2rem"
...
<style>
    ...
    .top-right-audio button,
    .top-right-audio button *,
    .top-right-audio button p,
    .top-right-audio button span {{
        color: #1B5E20 !important;
        font-size: {"1.05rem" if is_elder else "0.85rem"} !important;
    ...
    .top-right-sos button,
    .top-right-sos button *,
    .top-right-sos button p,
    .top-right-sos button span {{
...
# ==================== 4. 頂部標題列、2 個小功能與關愛大字體開關 ====================
col_title, col_small_features = st.columns([1.2, 1.1])

with col_title:
    title_font_size = "1.8rem" if is_elder else "1.5rem"
    sub_title_size = "1.1rem" if is_elder else "0.88rem"
    st.markdown(f"""
    <div style="text-align: left; padding: 0px;">
        <div style="font-size: {title_font_size}; font-weight: 800; color: #1B5E20; letter-spacing: -0.5px; display: flex; align-items: center; gap: 4px;">
            <span>🍀</span> <span>絲野仙蹤 Eco-Family</span>
        </div>
        <div style="font-size: {sub_title_size}; color: #2E7D32; margin-top: 2px; font-weight: 600;">
            澳門親子綠色呼吸智慧隨行助手
        </div>
    </div>
    """, unsafe_allow_html=True)
...
with col_small_features:
    ...
    with sub_col1:
        ...
        audio_status_text = "🔊 🟢 驅蟲中" if st.session_state.audio_active else "🔊 🔴 驅蟲未啟"
...
if st.session_state.current_page == "menu":
    ...
    # 3. 大功能三：多頻率驅蚊驅蟲器
    audio_btn_text = "🔊 多頻率驅蚊驅蟲器 (運行中)" if st.session_state.audio_active else "🔊 多頻率驅蚊驅蟲器"
    if st.button(audio_btn_text, key="btn_m3", use_container_width=True):
        st.session_state.current_page = "audio"
        st.rerun()
...
# ==================== 6. 大功能 1：🗺️ 智慧路線規劃 ====================
    ...
    <div class="card">
        ...
        <p style="margin-bottom:0; color:#2E7D32;">根據坡度需求、母嬰室設施與氣象自動進行最佳路線推薦：</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_sel2:
        selected_slope = st.selectbox("🏔️ 坡度篩選：", ["全部坡度", "平緩 (斜行電梯/無障礙)", "平緩道路", "中等緩坡"])

    need_nursery = st.checkbox("🍼 僅顯示設有母嬰室設施之路線", value=False)
    ...
    for idx, route in enumerate(dest_routes):
        ...
        is_best = (idx == 0)
        badge = '<span class="badge-star">🌟 首選推薦</span>' if is_best else '<span class="badge-green">推薦</span>'
...
# ==================== 7. 大功能 2：🎒 隨行裝備 ====================
    ...
    st.markdown("""
    <div class="card">
        <h3 style="margin-top:0px; color:#1B5E20;">🎒 當前氣象動態推薦隨行裝備</h3>
        <p style="margin-bottom:0; color:#2E7D32;">系統依據最新氣溫、紫外線與氣象資料自動生成之準備清單：</p>
    </div>
    """, unsafe_allow_html=True)

    st.markdown("##### 📌 出行必備基礎裝備")
    st.checkbox("🍼 **兒童水壺 / 保溫杯**", value=False, key="gear_water")
    st.checkbox("🧻 **濕紙巾與個人衛生用品**", value=False, key="gear_wipes")
    ...
    if st.session_state.global_rain:
        ...
        st.checkbox("🌂 **備用乾爽衣物 1 套**", value=False, key="gear_rain2")

    if st.session_state.global_uv >= 2.0:
        st.markdown("##### ☀️ 防曬護膚專屬裝備")
        st.checkbox("☀️ **兒童高效防曬乳 (SPF50+)**", value=False, key="gear_uv_1")
        st.checkbox("🧢 **親子大簷遮陽帽**", value=False, key="gear_uv_2")

    if st.session_state.global_temp >= 25.0:
        st.markdown("##### 🌡️ 高溫防暑專屬裝備")
        st.checkbox("🌬️ **便攜推車小風扇**", value=False, key="gear_temp_1")
...
# ==================== 8. 大功能 3：🔊 多頻率驅聲波 ====================
    ...
    st.components.v1.html(audio_js, height=80)
...
# ==================== 9. 小功能 2 頁面：🚨 一鍵求救 ====================
    ...
    st.components.v1.html(sos_js_template, height=320)